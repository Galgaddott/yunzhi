			...
			//获取主体
			 ...
			 if(!subject.isAuthenticated() && !subject.isRemembered()) {
				 System.out.println("正在进入登录区");
					//封装账户信息 对密码先进行md5加密
						UsernamePasswordToken token = new UsernamePasswordToken((String)str.get("userName"),EncryptKit.MD5((String)str.get("passWord")));
						...
						try {
							...
							//进行正则判断是用 用户名还是邮箱登录
							Matcher matcher= Pattern.compile("\\w{3,20}\\@\\w+\\.((com)|(cn))").matcher((String)str.get("userName"));
							
							if(matcher.matches()) 
								Email = (String)str.get("userName");
							else Name = (String)str.get("userName");
							
							User user = userService.login(Name,(String)str.get("passWord"), Email);
...
//	            
//	            mMessageHelper.setSubject("Scratch在线交流平台验证码");//邮件的主题
//	          
